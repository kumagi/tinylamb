...
namespace tinylamb {
namespace {
const char* kTableName = "SampleTable";
}  // namespace
...
TEST_F(IndexTest, Construct_Default_Succeeds) {}

TEST_F(IndexTest, Insert_DuplicateRowsOnNonUniqueIndex_Succeeds) {
  TransactionContext ctx = rs_->BeginContext();
...
TEST_F(IndexTest, Read_AfterInsert_ReturnsInsertedRow) {
  TransactionContext ctx = rs_->BeginContext();
...
TEST_F(IndexTest, Update_SingleRow_UpdatesRowSuccessfully) {
  TransactionContext ctx = rs_->BeginContext();
  ASSIGN_OR_ASSERT_FAIL_CONST(std::shared_ptr<Table>, tbl,
                              ctx.GetTable(kTableName));
...
TEST_F(IndexTest, Update_MultipleRowsRepeatedly_UpdatesSuccessfully) {
  TransactionContext ctx = rs_->BeginContext();
  ASSIGN_OR_ASSERT_FAIL_CONST(std::shared_ptr<Table>, tbl,
                              ctx.GetTable(kTableName));
...
TEST_F(IndexTest, Delete_ExistingRow_MakesRowUnreadable) {
  TransactionContext ctx = rs_->BeginContext();
  ASSIGN_OR_ASSERT_FAIL_CONST(std::shared_ptr<Table>, tbl,
                              ctx.GetTable(kTableName));
  ASSIGN_OR_ASSERT_FAIL(
...
TEST_F(IndexTest, Insert_MultipleRowsWithIndex_Succeeds) {
  TransactionContext ctx = rs_->BeginContext();
  ASSIGN_OR_ASSERT_FAIL_CONST(std::shared_ptr<Table>, tbl,
                              ctx.GetTable(kTableName));
...
TEST_F(IndexTest, Update_RowWithIndex_PreservesRowPosition) {
  TransactionContext ctx = rs_->BeginContext();
  ASSIGN_OR_ASSERT_FAIL_CONST(std::shared_ptr<Table>, tbl,
                              ctx.GetTable(kTableName));
...
TEST_F(IndexTest, Delete_RowWithIndex_PreservesRemainingRowPositions) {
  TransactionContext ctx = rs_->BeginContext();
  ASSIGN_OR_ASSERT_FAIL_CONST(std::shared_ptr<Table>, tbl,
                              ctx.GetTable(kTableName));
...
TEST_F(IndexTest, Insert_ManyRows_AllRowsAreReadable) {
  TransactionContext ctx = rs_->BeginContext();
  ASSIGN_OR_ASSERT_FAIL_CONST(std::shared_ptr<Table>, tbl,
                              ctx.GetTable(kTableName));
...
TEST_F(IndexTest, Update_ManyRowsUnderHeavyLoad_UpdatesSuccessfully) {
  ...
  for (int i = 0; i < kCount * 4; ++i) {
    ...
    std::string key = RandomString(((static_cast<size_t>(19937) * i) % 3200) + 500, false);
