...
- **`TableStatistics`**: A persistent, versioned data structure containing the table row count and per-column NULL count, non-NULL count, distinct cardinality, 16 equi-depth histogram buckets, five lowest and highest distinct values, five most-common values with frequencies, and a physical-order correlation in [0,1] (fraction of adjacent non-NULL pairs in scan order that are non-decreasing; old payloads without the field decode as 1.0). The catalog stores row count under the table name and each `ColumnStats` blob under a separate B+tree key so a wide table cannot exceed the leaf entry size. Stored string boundaries use a bounded prefix so statistics cannot grow with unbounded VARCHAR payloads. Legacy min/max statistics remain readable and are upgraded by the next refresh.
  ...
  - **Selectivity Estimation**: Equality and range predicates use most-common values and histogram interpolation. `AND`, `OR`, `XOR`, `IN`, `IS NULL`, `IS NOT NULL`, mixed numeric constants, and same-column correlated predicates have explicit probability rules rather than fixed density constants.
