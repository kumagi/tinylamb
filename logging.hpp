...
public:
 Logging(std::string_view filename)
  ...
  bool Append(uint64_t lsn, std::string_view payload) {
    ...
    dst_.write(reinterpret_cast<const char*>(&payload_size), sizeof(payload_size));
  ...
  bool Read(uint64_t lsn, std::string* payload) {
    ...
    while (dst_.good()) {
      ...
      if (current_lsn == lsn) {
        ...
        dst_.read(reinterpret_cast<char*>(&content_length), sizeof(content_length));
