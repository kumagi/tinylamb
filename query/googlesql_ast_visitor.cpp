...
SelectSource ExpandUnpivotSource(SelectSource base,
...
std::string DecodeSingleComponent(std::string_view value_view) {
  ...
  for (size_t i = 0; i < value.size(); ++i) {
    ...
    } else if (value[i] == quote && i + 1 < value.size() &&
               value[i + 1] == quote) {
...
SelectSource ExpandPivotSource(SelectSource base,
  ...
  for (const GoogleSqlAstNode* pv : value_list->Children("PivotValue")) {
    ...
    for (const auto& agg : aggs) {
      std::string col_name;
      if (aggs.size() == 1 && agg.agg_alias.empty()) {
        col_name = val_alias;
      } else if (agg.agg_alias.empty()) {
        col_name = val_alias;
...
SelectSource VisitTableSource(
  ...
  if (const GoogleSqlAstNode* pivot = node.Child("PivotClause")) {
    ...
    source = ExpandUnpivotSource(std::move(source), *unpivot);
  }
  return source;
}
