...
const std::vector<std::vector<std::string>>& SeedPool() {
  static const std::vector<std::vector<std::string>>* pool = new std::vector<
      ...
      // Outer joins with NULL keys on both sides.
      {
          ...
          "INSERT INTO l VALUES (1, 'a'), (2, NULL), (NULL, "
          "'c'), "
          "(2, 'd');",
          "CREATE TABLE r (rk INT64, rv INT64);",
          "INSERT INTO r VALUES (2, 20), (2, 21), (NULL, 0), "
          "(3, "
          "30);",
          "SELECT l.lk, r.rv FROM l LEFT JOIN r ON l.lk = r.rk "
          "ORDER BY "
          "l.lk;",
          "SELECT l.lk, r.rv FROM l RIGHT JOIN r ON l.lk = "
          "r.rk "
          "ORDER BY "
          "r.rv;",
          "SELECT l.lk, r.rv FROM l FULL JOIN r ON l.lk = "
          "r.rk;",
          "SELECT COUNT(*) FROM l LEFT JOIN r ON l.lk = r.rk "
          "WHERE "
          "r.rv "
          "IS NULL;",
          "SELECT COALESCE(l.lv, '?') v, r.rv FROM l LEFT JOIN "
          "r "
          "ON "
          "l.lk = r.rk;",
      },
      // Set operations.
      {
          ...
          "SELECT x FROM s1 UNION ALL SELECT x FROM s2 ORDER "
          "BY x;",
      ...
      // Window functions.
      {
          ...
          "INSERT INTO w VALUES (1, 10, 'a'), (1, NULL, 'b'), "
          "(2, "
          "30, 'a'), "
          "(2, 30, 'c'), (NULL, 5, NULL);",
          "SELECT g, v, ROW_NUMBER() OVER (ORDER BY v) rn FROM "
          "w;",
          "SELECT g, v, RANK() OVER (PARTITION BY g ORDER BY "
          "v) rk "
          "FROM w;",
      ...
      // CTE + correlated and uncorrelated subqueries.
      {
          ...
          "INSERT INTO c VALUES (1, 1, 10), (2, 1, NULL), (3, "
          "2, "
          "30), "
          "(4, NULL, 5);",
          "WITH big AS (SELECT grp, SUM(val) sv FROM c GROUP "
          "BY "
          "grp) "
          "SELECT c.id FROM c JOIN big ON c.grp = big.grp "
          "WHERE "
          "big.sv > "
          "20;",
          "SELECT id FROM c WHERE grp IN (SELECT grp FROM c "
          "WHERE "
          "val > "
          "20);",
          "SELECT id FROM c WHERE EXISTS (SELECT 1 FROM c d "
          "WHERE "
          "d.grp = "
      ...
      // GROUPING SETS / FILTER / DISTINCT aggregates.
      {
          ...
          "INT64);",
          "INSERT INTO gs VALUES ('e','p1',3), "
          "('w','p2',NULL), "
          "('e','p2',"
          "7), ('e','p1',-2);",
          "SELECT region, SUM(qty) FROM gs GROUP BY region;",
          "SELECT region, COUNT(DISTINCT product) FROM gs "
          "GROUP BY "
          "region;",
      ...
      // CASE / COALESCE / NULLIF / boolean 3VL stress.
      {
          ...
          "NULL), (0, "
          "'');",
          "SELECT i, CASE WHEN i > 0 THEN 'pos' WHEN i < 0 "
          "THEN "
          "'neg' "
          "WHEN i IS NULL THEN 'null' ELSE 'zero' END c FROM "
          "cs;",
          "SELECT COALESCE(s, 'dflt'), NULLIF(i, 0) FROM cs;",
          "SELECT i FROM cs WHERE (i > 0) OR (s IS NULL);",
          "SELECT i FROM cs WHERE NOT (i > 0 AND s IS NOT "
          "NULL);",
      ...
      // UPDATE/DELETE with expressions and subquery predicates.
      {
          ...
          "INSERT INTO dm VALUES (1, 100, 'a'), (2, -5, 'b'), "
          "(3, "
          "NULL, "
          "'a'), (4, 7, NULL);",
          "UPDATE dm SET v = v * 2 WHERE tag = 'a';",
          "UPDATE dm SET v = NULL WHERE v < 0;",
          "DELETE FROM dm WHERE id IN (SELECT id FROM dm WHERE "
          "v "
          "IS NULL);",
          "SELECT COUNT(*), MIN(v), MAX(v) FROM dm;",
          "UPDATE dm SET tag = 'z' WHERE v > (SELECT MIN(v) "
          "FROM "
          "dm);",
      ...
      {
          "CREATE TABLE tree (id INT64, parent INT64, w "
          "DOUBLE);",
          "INSERT INTO tree VALUES (1, NULL, 1.0), (2, 1, "
          "2.5), "
          "(3, 1, "
          "2.5), (4, 2, NULL), (5, 5, 0.0);",
          "SELECT c.id, p.id FROM tree c JOIN tree p ON "
          "c.parent = "
          "p.id "
          "ORDER BY c.id;",
          "SELECT a.id, b.id FROM tree a CROSS JOIN tree b "
          "WHERE "
          "a.id = "
          "b.id;",
          "SELECT t.id FROM tree t LEFT JOIN tree k ON "
          "t.parent = "
          "k.id "
          "WHERE k.id IS NULL;",
      ...
      },
      {
          ...
          "INSERT INTO t1 (a, b, c) VALUES (1, 'x', 1.5), (2, "
          "'y', "
          "NULL), "
          "(NULL, 'z', 3.25);",
          "SELECT a * 3 + 1 FROM t1 WHERE a > 0;",
          "SELECT b, COUNT(*) FROM t1 GROUP BY b;",
          "SELECT t1.a, t2.d FROM t1 JOIN t2 ON t1.a = t2.d "
          "ORDER "
          "BY "
          "t1.a;",
      },
      {
          ...
          "INSERT INTO t2 VALUES (1, 'p'), (2, NULL), (3, "
          "'q');",
      ...
      },
      {
          ...
          "INSERT INTO emp VALUES (1, NULL, 100), (2, 1, 50), "
          "(3, "
          "1, 70), "
          "(4, 2, NULL);",
          "SELECT e.id FROM emp e JOIN emp m ON e.mgr = m.id "
          "WHERE "
          "m.sal "
          "> 60;",
  ...
  try {
    ...
    for (const std::string_view known :
         {"not found", "no such", "ambiguous column", "numeric value",
          "overflow", "cannot access field", "cannot be compared",
          "Different type", "Unknown type", "requires an integer",
          "requires a number"}) {
