...
class SqlEngineTpccTest : public ::testing::Test {
 protected:
  ...
  static std::vector<Row> Run(Database& database, TransactionContext& context,
    ...
    std::vector<Row> rows;
    Row row;
    while (result.Value()->Next(&row, nullptr)) {
      rows.push_back(row);
    }
    return rows;
  }
...
TEST_F(SqlEngineTpccTest, ExecutesAllTransactionQueryShapes) {
  ...
  Run(*database_, context,
      "INSERT INTO district VALUES (1, 1, 0.2, 50.0, 101);");
  Run(*database_, context,
  ...
  Run(*database_, context,
      ...
      "(2, 1, 8, 0.0, 0, 0, 'plain');");
  Run(*database_, context,
      "INSERT INTO orders VALUES (100, 1, 1, 1, NULL, 2);");
  Run(*database_, context, "INSERT INTO new_order VALUES (100, 1, 1);");
  Run(*database_, context,
  ...
  EXPECT_EQ(Run(*database_, context,
                "SELECT w_tax FROM warehouse WHERE w_id = 1;")[0][0],
            Value(0.1));
  Run(*database_, context,
  ...
  Run(*database_, context,
      "INSERT INTO orders VALUES (101, 1, 1, 1, NULL, 1);");
  Run(*database_, context, "INSERT INTO new_order VALUES (101, 1, 1);");
  Run(*database_, context,
      "INSERT INTO order_line VALUES "
  ...
  Run(*database_, context,
      "UPDATE warehouse SET w_ytd = w_ytd + 10.0 WHERE w_id = 1;");
  Run(*database_, context,
  ...
  auto customers = Run(*database_, context,
  ...
  auto lines = Run(*database_, context,
  ...
  auto oldest = Run(*database_, context,
  ...
  Run(*database_, context,
      ...
      "WHERE ol_w_id = 1 AND ol_d_id = 1 AND ol_o_id = 100;");
  auto amount = Run(*database_, context,
  ...
  auto low_stock = Run(*database_, context,
