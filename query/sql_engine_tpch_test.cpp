...
TEST_F(SqlEngineTpchTest, TpchQ8Q9JoinOrderGolden) {
  ...
  // Relations: part, supplier, lineitem, orders, customer, nation n1, nation
  // n2, region.
  ...
  std::vector<Row> q8_rows = Run(*database_, context, kTpchQueries[7]);
  ASSERT_EQ(q8_rows.size(), 1U);
  EXPECT_EQ(q8_rows[0][0], Value(1994));
  EXPECT_NEAR(q8_rows[0][1].value.double_value, 1.0, 1e-6);

  std::vector<Row> q9_rows = Run(*database_, context, kTpchQueries[8]);
