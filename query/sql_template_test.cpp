...
TEST(SqlTemplateTest, TripleQuotedStringsExtractedCorrectly) {
  const SqlTemplate single = ExtractSqlTemplate("SELECT '''abc\\ndef''';");
  ...
  const SqlTemplate multiline = ExtractSqlTemplate("SELECT '''abc\ndef''';");
  ...
  const SqlTemplate unescaped = ExtractSqlTemplate("SELECT '''a'b\"''';");
