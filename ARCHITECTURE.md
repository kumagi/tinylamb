...
> 設計判断の記録 (ADR) は [docs/review/](docs/review/) と
> [docs/adr-proposal.md](docs/adr-proposal.md) (`docs/adr/` ディレクトリは
> 実在しない。ADR を常設する場合はそこに `NNN-<topic>.md` で作る)。

この文書は「今どこを触っているのか」「新しい変更はどの層に属するのか」の共通言語を
提供する。依存方向の強制は `scripts/check_layering.py` (include-lint) が行う
(`CMakeLists.txt` の `tinylamb_add_layer` 宣言と対応)。
...
sql                   query/           (GoogleSQL frontend, SqlEngine,
                       statement IR 本体は query/statement.hpp)
  ↓
...
| 式の意味論・演算子追加 | expression | type, … | **AST 直評価を先に**更新し bytecode 経路も揃える (differential test) |
| JIT 生成コードの変更 | expression (`jit.cpp`) | 同上 | AST を正本・bytecode を IR として両方に揃えること |
