...
TEST(FunctionCallAstTest, MathFunctionsAstParity) {
  ...
  auto eval = [&](const std::string& name, const std::vector<Value>& args) {
    ...
    return FunctionCallExp(name, std::move(exprs))
  ...
  EXPECT_DOUBLE_EQ(
      eval("pow", {Value(2.0), Value(3.0)}).Value().value.double_value, 8.0);
  ...
  EXPECT_DOUBLE_EQ(
      eval("atan2", {Value(0.0), Value(1.0)}).Value().value.double_value, 0.0);
  ...
  EXPECT_DOUBLE_EQ(eval("radians", {Value(180.0)}).Value().value.double_value,
                   M_PI);
  EXPECT_DOUBLE_EQ(eval("degrees", {Value(M_PI)}).Value().value.double_value,
                   180.0);
  EXPECT_DOUBLE_EQ(eval("pi", {}).Value().value.double_value, M_PI);
  EXPECT_EQ(eval("mod", {Value(int64_t{10}), Value(int64_t{3})}).Value(),
            Value(int64_t{1}));
  EXPECT_EQ(eval("div", {Value(int64_t{10}), Value(int64_t{3})}).Value(),
            Value(int64_t{3}));
  EXPECT_EQ(eval("ieee_divide", {Value(1.0), Value(2.0)}).Value(), Value(0.5));
  EXPECT_DOUBLE_EQ(
      eval("safe_divide", {Value(8.0), Value(2.0)}).Value().value.double_value,
      4.0);
}
...
TEST(FunctionCallAstTest, TypeSafetyAndDomainErrors) {
  ...
  EXPECT_EQ(eval("sqrt", {Value("not_a_number")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("cbrt", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("ln", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("log", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("cos", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("sin", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("tan", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("acos", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("asin", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("atan", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("atan2", {Value("text"), Value(1.0)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("atan2", {Value(1.0), Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("pow", {Value("text"), Value(2.0)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("pow", {Value(2.0), Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("ceil", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("floor", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("round", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("trunc", {Value("text")}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("div", {Value("text"), Value(2)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("ieee_divide", {Value("text"), Value(2.0)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("safe_divide", {Value("text"), Value(2.0)}).GetStatus(),
            StatusCode::kInvalidArgument);

  // Domain error validation
  EXPECT_EQ(eval("sqrt", {Value(-1.0)}).GetStatus(),
            StatusCode::kInvalidArgument);
  ...
  EXPECT_DOUBLE_EQ(
      eval("log", {Value(10.0), Value(0.0)}).Value().value.double_value, 0.0);
  EXPECT_TRUE(
      std::isinf(
          eval("log", {Value(10.0), Value(1.0)}).Value().value.double_value) ||
      std::isnan(
          eval("log", {Value(10.0), Value(1.0)}).Value().value.double_value));
  EXPECT_EQ(eval("acos", {Value(2.0)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("acos", {Value(-2.0)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("asin", {Value(2.0)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("asin", {Value(-2.0)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("div", {Value(int64_t{10}), Value(int64_t{0})}).GetStatus(),
            StatusCode::kIsInfinity);
  EXPECT_EQ(eval("div", {Value(std::numeric_limits<int64_t>::min()),
                         Value(int64_t{-1})})
                .GetStatus(),
            StatusCode::kIsInfinity);
  EXPECT_EQ(
      eval("div", {Value(std::numeric_limits<double>::quiet_NaN()), Value(2.0)})
          .GetStatus(),
      StatusCode::kIsInfinity);
  EXPECT_EQ(
      eval("div", {Value(1.0), Value(std::numeric_limits<double>::quiet_NaN())})
          .GetStatus(),
      StatusCode::kIsInfinity);
  EXPECT_EQ(
      eval("div", {Value(std::numeric_limits<double>::infinity()), Value(2.0)})
          .GetStatus(),
      StatusCode::kIsInfinity);
  EXPECT_EQ(eval("pow", {Value(-1.0), Value(0.5)}).GetStatus(),
            StatusCode::kInvalidArgument);
  EXPECT_EQ(eval("pow", {Value(0.0), Value(-1.0)}).GetStatus(),
            StatusCode::kIsInfinity);
  EXPECT_EQ(eval("pow", {Value(2.0), Value(1024.0)}).GetStatus(),
            StatusCode::kIsInfinity);
  EXPECT_EQ(eval("power", {Value(-1.0), Value(0.5)}).GetStatus(),
            StatusCode::kInvalidArgument);
...
TEST(FunctionCallAstTest, FoldMathFunctions) {
  Expression pow_expr = FunctionCallExp(
      "pow", {ConstantValueExp(Value(2.0)), ConstantValueExp(Value(3.0))});
  Expression rewritten_pow =
      ExpressionRewriter(ExpressionRuleSet::Default()).Rewrite(pow_expr);
  ASSERT_EQ(rewritten_pow->Type(), TypeTag::kConstantValue);
  EXPECT_DOUBLE_EQ(
      rewritten_pow->AsConstantValue().GetValue().value.double_value, 8.0);

  Expression sqrt_expr =
      FunctionCallExp("sqrt", {ConstantValueExp(Value(16.0))});
  Expression rewritten_sqrt =
      ExpressionRewriter(ExpressionRuleSet::Default()).Rewrite(sqrt_expr);
  ASSERT_EQ(rewritten_sqrt->Type(), TypeTag::kConstantValue);
  EXPECT_DOUBLE_EQ(
      rewritten_sqrt->AsConstantValue().GetValue().value.double_value, 4.0);

  Expression ceil_expr =
      FunctionCallExp("ceil", {ConstantValueExp(Value(2.1))});
  Expression rewritten_ceil =
      ExpressionRewriter(ExpressionRuleSet::Default()).Rewrite(ceil_expr);
  ASSERT_EQ(rewritten_ceil->Type(), TypeTag::kConstantValue);
  EXPECT_DOUBLE_EQ(
      rewritten_ceil->AsConstantValue().GetValue().value.double_value, 3.0);
}

TEST(FunctionCallAstTest, ResultTypeMixedNumeric) {
  Schema dummy_schema;
  Expression safe_add_int = FunctionCallExp(
      "safe_add", {ConstantValueExp(Value(1)), ConstantValueExp(Value(2))});
  EXPECT_EQ(safe_add_int->ResultType(dummy_schema).GetType(), TypeTag::kBigInt);
  EXPECT_EQ(safe_add_int->ResultType(dummy_schema, dummy_schema).GetType(),
            TypeTag::kBigInt);

  Expression safe_add_mixed = FunctionCallExp(
      "safe_add", {ConstantValueExp(Value(1)), ConstantValueExp(Value(2.5))});
  EXPECT_EQ(safe_add_mixed->ResultType(dummy_schema).GetType(),
            TypeTag::kDouble);
  EXPECT_EQ(safe_add_mixed->ResultType(dummy_schema, dummy_schema).GetType(),
            TypeTag::kDouble);

  Expression safe_sub_mixed = FunctionCallExp(
      "safe_subtract",
      {ConstantValueExp(Value(1)), ConstantValueExp(Value(2.5))});
  EXPECT_EQ(safe_sub_mixed->ResultType(dummy_schema).GetType(),
            TypeTag::kDouble);
  EXPECT_EQ(safe_sub_mixed->ResultType(dummy_schema, dummy_schema).GetType(),
            TypeTag::kDouble);

  Expression safe_mul_mixed = FunctionCallExp(
      "safe_multiply",
      {ConstantValueExp(Value(1)), ConstantValueExp(Value(2.5))});
  EXPECT_EQ(safe_mul_mixed->ResultType(dummy_schema).GetType(),
            TypeTag::kDouble);
  EXPECT_EQ(safe_mul_mixed->ResultType(dummy_schema, dummy_schema).GetType(),
            TypeTag::kDouble);

  Expression mod_int = FunctionCallExp(
      "mod", {ConstantValueExp(Value(5)), ConstantValueExp(Value(2))});
  EXPECT_EQ(mod_int->ResultType(dummy_schema).GetType(), TypeTag::kBigInt);
  EXPECT_EQ(mod_int->ResultType(dummy_schema, dummy_schema).GetType(),
            TypeTag::kBigInt);

  Expression mod_mixed1 = FunctionCallExp(
      "mod", {ConstantValueExp(Value(5)), ConstantValueExp(Value(2.5))});
  EXPECT_EQ(mod_mixed1->ResultType(dummy_schema).GetType(), TypeTag::kDouble);
  EXPECT_EQ(mod_mixed1->ResultType(dummy_schema, dummy_schema).GetType(),
            TypeTag::kDouble);

  Expression mod_mixed2 = FunctionCallExp(
      "mod", {ConstantValueExp(Value(5.5)), ConstantValueExp(Value(2))});
  EXPECT_EQ(mod_mixed2->ResultType(dummy_schema).GetType(), TypeTag::kDouble);
  EXPECT_EQ(mod_mixed2->ResultType(dummy_schema, dummy_schema).GetType(),
            TypeTag::kDouble);
}
...
TEST(FunctionCallAstTest, StringScalarParity) {
  ...
  // ascii / unicode
  auto ascii_val = FunctionCallExp("ascii", {ConstantValueExp(Value("ABC"))})
                       ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(ascii_val, Value(int64_t{65}));

  auto unicode_val = FunctionCallExp("unicode", {ConstantValueExp(Value("€"))})
                         ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(unicode_val, Value(int64_t{8364}));

  // chr
  auto chr_val = FunctionCallExp("chr", {ConstantValueExp(Value(int64_t{65}))})
                     ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(chr_val, Value("A"));

  // reverse / byte_reverse
  auto rev_val = FunctionCallExp("reverse", {ConstantValueExp(Value("hello"))})
                     ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(rev_val, Value("olleh"));

  // repeat
  auto rep_val =
      FunctionCallExp("repeat", {ConstantValueExp(Value("ab")),
  ...
  // initcap
  auto init_val =
      FunctionCallExp("initcap", {ConstantValueExp(Value("hello world"))})
          ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(init_val, Value("Hello World"));

  // translate
  auto trans_val =
      FunctionCallExp("translate", {ConstantValueExp(Value("12345")),
                                    ConstantValueExp(Value("135")),
                                    ConstantValueExp(Value("abc"))})
          ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(trans_val, Value("a2b4c"));

  // lpad / rpad
  auto lpad_val = FunctionCallExp("lpad", {ConstantValueExp(Value("hi")),
                                           ConstantValueExp(Value(int64_t{5})),
                                           ConstantValueExp(Value("*"))})
                      ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(lpad_val, Value("***hi"));

  auto rpad_val = FunctionCallExp("rpad", {ConstantValueExp(Value("hi")),
                                           ConstantValueExp(Value(int64_t{5})),
                                           ConstantValueExp(Value("*"))})
                      ->Evaluate(dummy_row, dummy_schema);
  ...
  // left / right
  auto left_val = FunctionCallExp("left", {ConstantValueExp(Value("hello")),
                                           ConstantValueExp(Value(int64_t{2}))})
                      ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(left_val, Value("he"));

  auto right_val =
      FunctionCallExp("right", {ConstantValueExp(Value("hello")),
                                ConstantValueExp(Value(int64_t{2}))})
          ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(right_val, Value("lo"));

  // regexp_contains / regexp_replace
  auto re_val = FunctionCallExp("regexp_contains",
                                {ConstantValueExp(Value("hello world")),
                                 ConstantValueExp(Value("world"))})
                    ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(re_val, Value(true));

  auto rep_re_val =
      FunctionCallExp("regexp_replace", {ConstantValueExp(Value("hello 123")),
                                         ConstantValueExp(Value("[0-9]+")),
                                         ConstantValueExp(Value("world"))})
          ->Evaluate(dummy_row, dummy_schema);
...
TEST(FunctionCallTest, UnsignedInt64_MathematicalFunctions) {
  ...
  auto safe_add_ok = FunctionCallExp("safe_add", {ConstantValueExp(umax - u10),
  ...
  // FORMAT
  auto fmt_res = FunctionCallExp("format", {ConstantValueExp(Value("%u")),
                                            ConstantValueExp(umax)})
                     ->Evaluate(dummy_row, dummy_schema);
  EXPECT_EQ(fmt_res, Value("18446744073709551615"));

  auto fmt_t_res = FunctionCallExp("format", {ConstantValueExp(Value("%T")),
                                              ConstantValueExp(umax)})
                       ->Evaluate(dummy_row, dummy_schema);
...
TEST(FunctionCallTest, GenerateArray_AstEvaluation) {
  ...
  auto res1 =
      FunctionCallExp("generate_array", {ConstantValueExp(Value(int64_t{1})),
                                         ConstantValueExp(Value(int64_t{5}))})
          ->TryEvaluate(dummy_row, dummy_schema);
  ...
  auto res2 = FunctionCallExp("generate_array", {ConstantValueExp(Value(1.5)),
  ...
  auto res3 =
      FunctionCallExp("generate_array", {ConstantValueExp(Value(int64_t{5})),
  ...
  auto res_non_num =
      FunctionCallExp("generate_array", {ConstantValueExp(Value("a")),
  ...
  auto res_null =
      FunctionCallExp("generate_array", {ConstantValueExp(Value()),
                                         ConstantValueExp(Value(int64_t{5}))})
          ->TryEvaluate(dummy_row, dummy_schema);
...
TEST(FunctionCallTest, UnsignedMathAndScalarFunctions) {
  ...
  // GREATEST / LEAST
  auto gr_res =
      FunctionCallExp("greatest", {ConstantValueExp(u_max_val),
                                   ConstantValueExp(Value(int64_t{0}))})
          ->TryEvaluate(dummy_row, dummy_schema);
...
TEST_F(ScalarFunctionSqlTest, UnsignedMathAndScalarFunctionsSql) {
  ...
  EXPECT_EQ(static_cast<uint64_t>(abs_v.value.int_value),
  ...
  Value gr_v =
      Scalar("SELECT GREATEST(CAST(18446744073709551615 AS UINT64), 0)");
  EXPECT_TRUE(gr_v.IsUnsigned());
  EXPECT_EQ(static_cast<uint64_t>(gr_v.value.int_value),
  ...
  Value safe_sub_null = Scalar("SELECT SAFE_SUBTRACT(CAST(0 AS UINT64), 1)");
  EXPECT_TRUE(safe_sub_null.IsNull());

  Value safe_neg_null = Scalar("SELECT SAFE_NEGATE(CAST(5 AS UINT64))");
  EXPECT_TRUE(safe_neg_null.IsNull());

  Value safe_neg_zero = Scalar("SELECT SAFE_NEGATE(CAST(0 AS UINT64))");
