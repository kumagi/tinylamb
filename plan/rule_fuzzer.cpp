  ...
  switch (flavor) {
    ...
    case 10: {
      ...
      static const std::array<const char*, 6> kPreds = {
          "t1.val IN (10, 20, 40)", "t1.val IN (10, NULL, 20)",
          "t1.val NOT IN (10, 20)", "t1.val NOT IN (10, NULL, 20)",
          "t1.flag IN (0, 1)",      "t1.flag NOT IN (1)",
      };
      predicate = kPreds[rng() % kPreds.size()];
      break;
    }
    ...
    case 12: {
      ...
      from_clause =
          "(SELECT id, val, flag, ROW_NUMBER() OVER (PARTITION BY flag ORDER "
          "BY "
    ...
    case 17: {
      ...
      static const std::array<const char*, 4> kPreds = {
          ...
          "NOT EXISTS (SELECT 1 FROM t2 WHERE t2.t1_id = t1.id AND t2.val IS "
          "NULL)",
          "EXISTS (SELECT 1 FROM t2 JOIN t3 ON t2.id = t3.t2_id WHERE t2.t1_id "
          "= t1.id)",
      };
      predicate = kPreds[rng() % kPreds.size()];
      break;
    ...
    case 19: {
      ...
      from_clause =
          "(SELECT t1.id, SUM(t2.val) AS total_val, COUNT(t2.id) AS cnt FROM "
          "t1 "
          "LEFT JOIN t2 ON t1.id = t2.t1_id GROUP BY t1.id HAVING COUNT(t2.id) "
          ">= 1) AS agg";
      static const std::array<const char*, 4> kPreds = {
          "total_val > 20",
          "total_val IS NULL",
    ...
    case 22: {
      ...
      from_clause = "t1";
      static const std::array<const char*, 3> kPreds = {
          "(CASE WHEN t1.val = 0 OR t1.val IS NULL THEN 0 ELSE 100 / t1.val "
          "END) > 2",
    ...
    case 23: {
      ...
      from_clause =
          "(SELECT id, val FROM t1 WHERE val > 0 UNION ALL SELECT t1_id AS id, "
          "val FROM t2 WHERE val IS NOT NULL) AS s";
      static const std::array<const char*, 3> kPreds = {
          "val > 15",
          "id = 1",
          "val = 10 OR val = 20",
...
void Worker(int thread_id, uint64_t base_seed,
  ...
  while (!stop_requested.load(std::memory_order_relaxed) &&
    ...
    if (pass == 0) {
      ...
      // Pass 6: SQL Oracle Metamorphic / Constraint Checks (TLP, PQS, IDX, DQE,
      // TROC)
      std::mt19937 sql_rng(static_cast<uint32_t>(rng()));
      OracleIterationStats o_stats;
      OracleTrace trace;
      const std::string problem =
          RunOracleIteration(sql_rng, false, &o_stats, &trace);
      if (!problem.empty()) {
        std::cerr << "\n[FAILURE in Worker " << thread_id
      ...
      GriffinTrace trace;
      const std::string problem =
          RunGriffinIteration(griffin_rng, false, &trace);
      if (!problem.empty()) {
        std::cerr << "\n[FAILURE in Worker " << thread_id
...
int main(int argc, char** argv) {
  ...
  std::cout
      ...
      << "             7. SQL Metamorphic & Constraint Oracles "
         "(TLP/PQS/IDX/DQE/TROC)\n"
  ...
  while (!stop_requested.load()) {
    ...
    const auto sql_cnt = stats.sql_oracle_runs.load(std::memory_order_relaxed);
    const auto amoeba_cnt = stats.amoeba_runs.load(std::memory_order_relaxed);
    const auto griffin_cnt = stats.griffin_runs.load(std::memory_order_relaxed);
    ...
    std::cout << "[" << std::setfill('0') << std::setw(2) << hh << ":"
              ...
              << " it/s) | NoREC: " << norec_cnt
              << " | ComplexMemo: " << complex_cnt
              << " | ComplexSubset: " << subset_cnt
              << " | ExprRule: " << expr_cnt << " | FullExpr: " << full_expr_cnt
              << " | RowNull: " << row_cnt << " | SqlOracle: " << sql_cnt
              << " | Amoeba: " << amoeba_cnt << " | Griffin: " << griffin_cnt
              << " | Failures: " << fails << "\n"
              << std::flush;
  }
