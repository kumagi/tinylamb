...
Value EvaluateFunction(  // NOLINT(misc-no-recursion)
  ...
  if (name == "if") {
    // IF is control flow: only the taken branch may be evaluated, so
    // ERROR() in the untaken branch stays dormant.
    if (call.Args().size() != 3) {
      throw std::runtime_error("IF requires 3 arguments");
    }
    const Value condition =
        Evaluate(call.Args()[0], scope, aggregates, context, ctes);
    return Evaluate(condition.Truthy() ? call.Args()[1] : call.Args()[2], scope,
                    aggregates, context, ctes);
  }
  if (name == "iserror" || name == "iferror" || name == "nulliferror") {
    // Error-handling wrappers evaluate their first argument defensively:
    // ISERROR reports whether it raised, IFERROR substitutes its second
    // argument on failure, and NULLIFERROR yields NULL on failure.
    if (call.Args().empty()) {
      throw std::runtime_error(name + " requires an argument");
    }
    Value try_value;
    bool errored = false;
    try {
      try_value = Evaluate(call.Args()[0], scope, aggregates, context, ctes);
    } catch (const std::exception&) {
      errored = true;
    }
    if (name == "iserror") {
      return Value(errored ? int64_t{1} : int64_t{0});
    }
    if (!errored) {
      return try_value;
    }
  ...
  switch (expression->Type()) {
    ...
    case TypeTag::kQueryExp: {
      ...
      if (value.ArrayResult()) {
        // ARRAY(SELECT ...): the whole subquery result becomes one array
        // value; an empty result yields an empty (non-NULL) array.
        std::vector<Value> elements;
        ...
        if (!elements.empty()) {
          for (const Value& element : elements) {
            if (!element.IsNull()) {
              if (element.IsArray()) {
                element_type = "ARRAY<" + element.ArrayElementSqlType() + ">";
              } else {
        ...
        } else if (relation->schema.ColumnCount() > 0) {
          element_type =
